...
class Document(models.Model):
    ...
    recipient_college = models.ForeignKey(College, on_delete=models.PROTECT, related_name='incoming_documents', verbose_name="الكلية المستقبلة")
    recipient_user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name='received_documents', null=True, blank=True, verbose_name="المستلم (الموظف المحدد)")
    
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='draft', verbose_name="حالة الخطاب")
    reference_number = models.CharField(max_length=50, unique=True, null=True, blank=True, verbose_name="الرقم المرجعي (الصادر)")
    parent = models.ForeignKey('self', on_delete=models.SET_NULL, null=True, blank=True, related_name='replies', verbose_name="الخطاب الأصلي")
    ...
    def generate_reference(self):
        ...
        if not self.reference_number and self.status == 'sent':
            ...
            count = Document.objects.filter(sender_college=self.sender_college, status='sent', created_at__year=year).count() + 1
            self.reference_number = f"{self.sender_college.code}-{self.recipient_college.code}-{year}-{count:04d}"

class Attachment(models.Model):
    document = models.ForeignKey(Document, on_delete=models.CASCADE, related_name='attachments', verbose_name="الخطاب")
...
class DocumentHistory(models.Model):
    ...
    document = models.ForeignKey(Document, on_delete=models.CASCADE, related_name='history', verbose_name="الخطاب")
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, verbose_name="المسؤول عن الإجراء")
...
class DocumentForward(models.Model):
    document = models.ForeignKey(Document, on_delete=models.CASCADE, related_name='forwards', verbose_name="الخطاب")
    department = models.ForeignKey(Department, on_delete=models.CASCADE, related_name='received_forwards', verbose_name="القسم الأكاديمي")
    forwarded_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, verbose_name="الموجّه")
