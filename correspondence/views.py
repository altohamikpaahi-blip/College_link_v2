...
class CreateDocumentView(LoginRequiredMixin, CreateView):
    ...
    def get_form(self, form_class=None):
        ...
        if user and user.college:
            # استبعاد كليتك من الإرسال
            form.fields['recipient_college'].queryset = College.objects.exclude(id=user.college.id)
            # استبعاد موظفي كليتك من قائمة المستلمين المحددين لمنع الأخطاء
...
class DocumentDetailView(LoginRequiredMixin, DetailView):
    """عرض تفاصيل الخطاب مع التحقق من صلاحيات الأطراف لحماية الخصوصية وحالة القراءة"""
    model = Document
    ...
    def get_object(self, queryset=None):
        ...
        # تحديث ذكي: إذا فتح المستلم المعاملة تتحول لمقروءة تلقائياً
...
def get_college_employees(request):
    """واجهة برمجية (API) تجلب الموظفين التابعين لكلية معينة بصيغة JSON"""
    college_id = request.GET.get('college_id')
    users = UserProfile.objects.filter(college_id=college_id).values('id', 'first_name', 'last_name', 'username')
    user_list = []
    for u in users:
